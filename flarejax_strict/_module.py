...
from ._config import ConfigMapping, is_valid_conifg_type
...
class FieldKey:
    ...
    grad: bool
    attr: bool = True
    meta: FrozenMappingHashable | ConfigMapping = dataclasses.field(
        repr=False,
        default_factory=lambda: FrozenMappingHashable({}),
    )
    conf: FrozenMappingHashable | ConfigMapping = dataclasses.field(
        repr=False,
        default_factory=lambda: FrozenMappingHashable({}),
    )
...
def is_leaf(value: Any) -> bool:
    return isinstance(value, (Module, jax.Array, np.ndarray))


def is_valid_field_type(obj: Any) -> bool:
    if is_valid_conifg_type(obj):
        return True
...
class Module(FrozenDataclassBase, register=False):
    ...
    def __post_init__(self: Self) -> None:
        for f in dataclasses.fields(type(self)):
            ...
            if not is_valid_field_type(value):
                error = f"Invalid field type for '{f.name}': {value}"
                raise TypeError(error)
